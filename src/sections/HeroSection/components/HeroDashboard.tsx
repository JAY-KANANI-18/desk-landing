...
export const HeroDashboard = () => {
  ...
  useEffect(() => {
    const updateScale = () => {
      const viewport = Math.max(window.innerWidth - 32, 280);
      const next = Math.min(0.5, Math.max(0.3, viewport / 980));
      setMobileScale(next);
    };
    updateScale();
  ...
  const dashboardCard = (
    <div className={`relative w-[980px] h-[514px] rounded-2xl bg-[#0d1422] border border-white/8 shadow-2xl shadow-black/50 overflow-hidden scan-glow transition-all duration-700 ${mounted ? "opacity-100 scale-100" : "opacity-0 scale-95"}`}>
      {/* Window chrome */}
            ...
            {conversations.map((c, i) => (
              ...
              >
                <div className="relative shrink-0">
                  ...
                  <div className="absolute -bottom-1 -right-1 w-4 h-4 rounded-full bg-white ring-1 ring-[#0d1422] shadow-sm flex items-center justify-center overflow-hidden">
                    <img src={c.channelIcon} alt={c.channel} className="w-3 h-3 object-contain" />
                  </div>
                </div>
        ...
        <div className="flex-1 flex flex-col">
          <div className="flex items-center gap-3 px-4 py-3 border-b border-white/5">
            <div className="relative">
              ...
              <div className="absolute -bottom-1 -right-1 w-4 h-4 rounded-full bg-white ring-1 ring-[#0d1422] shadow-sm flex items-center justify-center overflow-hidden">
                <img src={conversations[activeConv].channelIcon} alt={conversations[activeConv].channel} className="w-3 h-3 object-contain" />
              </div>
            </div>
            <div>
  ...
  return (
    <div
      className={`relative w-full max-w-[980px] mx-auto transition-all duration-1000 ease-out ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"}`}
    >
      {/* Ambient glow */}
      ...
      <div className="relative w-full md:hidden overflow-hidden" style={{ height: `${Math.round(514 * mobileScale)}px` }}>
        <div
          className="absolute top-0 left-1/2 origin-top"
          style={{ width: "980px", transform: `translateX(-50%) scale(${mobileScale})` }}
        >
          {dashboardCard}
        </div>
      </div>
      <div className="hidden md:block">{dashboardCard}</div>

      {/* Floating badges with animation */}
      {/* <div className={`absolute -left-16 bottom-1/4 bg-[#0d1422] border border-white/10 rounded-xl px-3 py-2.5 shadow-xl hidden lg:block animate-float`}>
